    ...
    public class WhisperBackendSentis : MonoBehaviour, IWhisperBackend
    {
        ...
        private IEnumerator LoadModels()
        {
            ...
            try
            {
                ...
                IsReady = true;
                Debug.Log("[WhisperManager] Whisper-Tiny models loaded (FR offline, Sentis).");
        ...
        private IEnumerator RunInference(float[] pcm, Action<string> callback)
        {
            ...
            for (int i = 0; i < MAX_NEW_TOKENS; i++)
            {
                ...
                int seqLen     = logits.shape.rank >= 2 ? logits.shape[logits.shape.rank - 2] : 1;
                int lastRowOff = (seqLen - 1) * vocabSize;
                int bestToken  = 0;
                float bestVal  = float.NegativeInfinity;
                for (int v = 0; v < vocabSize; v++)
                {
                    float val = logits[lastRowOff + v];
